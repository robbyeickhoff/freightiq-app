...
begin
  ...
  strong_distant_candidate_ids as (
    select s.id
    from public.mfi_stops as s
    where (
      pg_catalog.char_length(v_query) >= 4
      and pg_catalog.lower(s.name) like v_contains_pattern escape E'\\'
    )
      or (
        pg_catalog.char_length(v_query) >= 6
        and pg_catalog.lower(coalesce(s.address, '')) like v_contains_pattern escape E'\\'
      )
  ),
  candidate_ids as (
    select local_ids.id
    from local_candidate_ids as local_ids
    union
    select distant_ids.id
    from strong_distant_candidate_ids as distant_ids
  ),
  candidates as (
    select
      s.id as stop_id,
    ...
    join public.mfi_stops as s on s.id = ids.id
  ),
  scored as (
    select
      c.*,
      case
        ...
        when c.normalized_name like v_contains_pattern escape E'\\'
          or c.name_similarity >= 0.55::double precision then 3
      ...
      case
        ...
        when c.normalized_name like v_contains_pattern escape E'\\'
          then greatest(0.70::double precision, c.name_similarity)

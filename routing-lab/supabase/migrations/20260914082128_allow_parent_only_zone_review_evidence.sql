...
begin
  ...
  select v_user_id, p_route_id, evidence.stop_id, evidence.address, evidence.city,
    evidence.state, evidence.postal_code, evidence.address_key,
    coalesce(evidence.canonical_address_key, public.routing_lab_canonical_address_key(
  ...
  where (evidence.canonical_address_key is null or
      evidence.canonical_address_key = public.routing_lab_canonical_address_key(
        evidence.address, evidence.city, evidence.state, evidence.postal_code))
    and exists (
      ...
      where review.item ->> 'stopId' = evidence.stop_id
        ...
        and (review.item ->> 'selectedMicroZone') is not distinct from evidence.approved_micro_zone)
    and exists (

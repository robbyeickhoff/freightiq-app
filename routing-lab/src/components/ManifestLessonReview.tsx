...
function ManifestLessonReview({ route, onApprove, onReplay }: Props) {
  ...
  return (
    <section className="end-review" aria-labelledby="manifest-lesson-review-title">
      <div className="section-heading"><div><p className="eyebrow">End-of-route review</p><h2 id="manifest-lesson-review-title">Decide what Routing Lab should remember</h2></div><span className="review-badge">Route complete</span></div>
      {stage === 'choice' ? <div className="review-choice"><p>Your reasons were saved when the route changed. You do not need to enter them again. Review only whether a planned correction should become a reusable lesson.</p>{route.plannedCorrections.map((item, index) => <article className="meaningful-difference" key={item.recordedAt}><p className="insight-label">Correction {index + 1} · {approvedCorrections.includes(index) ? 'Lesson approved' : reviewedCorrections.includes(index) ? 'Not reusable' : 'Decision needed'}</p><h3>{item.description}</h3><p><strong>Reason already saved:</strong> {item.reasons.join(' · ')}</p>{item.note ? <p>“{item.note}”</p> : null}{!reviewedCorrections.includes(index) ? <div className="review-actions"><button className="primary-button" type="button" onClick={() => { setCorrectionIndex(index); setStage('draft') }}>Review What to Remember</button><button className="secondary-button" type="button" onClick={() => setReviewedCorrections((current) => [...current, index])}>Don’t Remember This</button></div> : null}</article>)}{(route.runState?.reasonRecords.length ?? 0) > 0 ? <div className="review-reasons"><h3>Route-time deviations already explained</h3>{route.runState?.reasonRecords.map((reason) => <article key={reason.recordedAt}><p>{reason.reasons.join(' · ')}</p>{reason.note ? <span>“{reason.note}”</span> : null}</article>)}</div> : null}{reviewedCorrections.length === route.plannedCorrections.length ? <div className="review-state-message review-state-message--success"><h3>Every planned correction reviewed</h3><p>{approvedCorrections.length} reusable {approvedCorrections.length === 1 ? 'lesson was' : 'lessons were'} approved. Saved route-time deviations remain attached to this run without asking for those reasons again.</p><button className="primary-button" type="button" onClick={() => void onReplay()}>Replay manifest with approved lessons</button></div> : null}</div> : null}
      {stage === 'draft' ? <div className="lesson-draft">
        <article className="meaningful-difference"><p className="insight-label">Correction being reviewed</p><h3>{correction?.description}</h3><p><strong>Reason already saved — no need to enter it again:</strong> {correction?.reasons.join(' · ')}</p></article>
        ...
        <div className="lesson-evidence"><strong>Saved operational reason</strong><p>{operationalReason}</p></div>
